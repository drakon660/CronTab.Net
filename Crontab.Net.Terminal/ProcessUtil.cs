...
namespace Crontab.Net.Terminal;
...
public static class ProcessUtil
{
    ...
    {
        return System.CommandLine.Invocation.Process.ExecuteAsync(command, args, workingDir, stdOut, stdErr,
        ...
        process.Exited += (_, e) =>
        {
            ...
            WithOutputLock(
                () =>
                {
                    ...
                    {
                        processLifetimeTask.TrySetException(new InvalidOperationException(
                            $"Command {filename} {arguments} returned exit code {process.ExitCode}. Standard error: \"{errorBuilder.ToString()}\""));
                    }
                    else
                    {
                        processLifetimeTask.TrySetResult(new ProcessResult(outputBuilder.ToString(),
    ...
    public static Task<ProcessResult> RunAsync(ProcessSpec processSpec, CancellationToken cancellationToken = default,
        bool throwOnError = true)
    {
        return RunAsync(processSpec.Executable!, processSpec.Arguments!, processSpec.WorkingDirectory,
            throwOnError: throwOnError, processSpec.EnvironmentVariables, processSpec.OutputData, processSpec.ErrorData,
            processSpec.OnStart, processSpec.OnStop, cancellationToken);
    }
